...
// Handle booking via POST or GET action
if( ( $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pid']) ) || (isset($_GET['action']) && $_GET['action']==='book' && isset($_GET['pid'])) ) { 
...
<head>
	...
	<style>
		...
		* {
		...
		.btn-book { position: relative; z-index: 9999;
			background: var(--gradient-primary);
			border: none;
			color: white;
			padding: 15px 40px;
		...
		/* Fix Book Now button not clickable */
		.btn-book {
		position: relative;
		...
		section, .section-padding, .pricing-section {
		position: relative;
		z-index: 0;
		}
		...
		/* AOS Fallback: keep content visible and clickable */
